...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("leading-tight font-semibold text-slate-900 dark:text-slate-100 text-lg", className)}
      {...props}
    />
  )
}
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("flex items-center px-6 [.border-t]:pt-6 [.border-t]:border-slate-200 dark:[.border-t]:border-slate-800", className)}
      {...props}
    />
  )
}
